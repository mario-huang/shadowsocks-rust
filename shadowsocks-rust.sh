...
check_ver_comparison(){
	...
	if [[ "${now_ver}" != "${sh_ver}" ]]; then
		echo -e "${Info} 发现 Shadowsocks Rust 已有新版本 [ ${sh_ver} ]，旧版本 [ ${now_ver} ]"
		...
		echo -e "${Info} 当前 Shadowsocks Rust 已是最新版本 [ ${sh_ver} ] ！" && exit 1
	fi
}
...
Download() {
	...
	wget --no-check-certificate -N "https://raw.githubusercontent.com/myeveryheart/shadowsocks-rust/main/shadowsocks-${sh_ver}.${arch}-unknown-linux-gnu.tar.xz"
...
Update_Shell(){
	...
	sh_new_ver=$(wget -qO- https://raw.githubusercontent.com/myeveryheart/shadowsocks-rust/main/shadowsocks-rust.sh | grep sh_ver | cut -d= -f2 | tr -d \")
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		[[ -z "${yn}" ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			wget -O shadowsocks-rust.sh --no-check-certificate https://raw.githubusercontent.com/myeveryheart/shadowsocks-rust/main/shadowsocks-rust.sh && chmod +x shadowsocks-rust.sh
...
action=$1
	echo && echo -e "  
==================================
shadowsocks-rust的一键安装脚本 ${Red_font_prefix}[${sh_ver}]${Font_color_suffix}
==================================
